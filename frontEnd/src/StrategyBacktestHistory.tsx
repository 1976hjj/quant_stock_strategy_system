    ...
    {selectedYear != null && available && <div className="history-trade-detail">
      {loading === selectedYear && !detail ? <p className="history-trade-note">正在读取 {selectedYear} 年成交记录…</p> : error ? <p className="history-trade-error">读取失败：{error}</p> : detail && <>
        {!detail.available ? <p className="history-trade-note">该历史回测未保存逐笔成交。</p> : <>
          ...
          <p className="history-trade-explain">本年净值收益按年初、年末账户净值计算；卖出已实现盈亏按股票自买入以来的移动平均成本计算并包含对应现金分红。仓位比例均为成交完成后的即时值：该股占总资产＝该股市值÷账户总资产，总仓位＝全部持仓市值÷账户总资产。</p>
          {detail.total === 0 ? <p className="history-trade-note">该年度没有实际成交。</p> : <div className="history-trade-table">
            <div className="history-trade-head"><span>日期 / 批次</span><span>股票</span><span>方向</span><span>股数</span><span>成交价</span><span>成交金额</span><span>费用明细</span><span>卖出已实现盈亏</span><span>交易后持仓</span><span>该股占总资产</span><span>总仓位</span></div>
            {detail.trades.map((trade, index) => <div className="history-trade-row" key={`${trade.session}-${trade.rebalance_id}-${trade.ts_code}-${trade.side}-${index}`}>
              ...
              <b>{number(trade.post_quantity, 0)} 股</b>
              {trade.post_security_weight != null
                ? <b className="trade-weight">{percent(trade.post_security_weight)}<small>市值 ¥{number(trade.post_security_value_cny, 2)}</small></b>
                : <span className="trade-na" title="旧回测没有保存逐笔仓位快照，请重新运行回测">需重跑</span>}
              {trade.post_total_position_weight != null
                ? <b className="trade-weight">{percent(trade.post_total_position_weight)}<small>持仓 ¥{number(trade.post_invested_value_cny, 2)}</small></b>
                : <span className="trade-na" title="旧回测没有保存逐笔仓位快照，请重新运行回测">需重跑</span>}
            </div>)}
          </div>}
...
function Detail({ job }: { job: StrategyJob }) {
  ...
  return <div className="history-detail">
    ...
    <section><h3>策略配置</h3><dl>
      ...
      <div><dt>持股 / 保留排名</dt><dd>{request?.target_count} / {request?.retention_rank}</dd></div>
      <div><dt>调仓间隔</dt><dd>{request?.rebalance_sessions} 个交易日</dd></div>
      <div><dt>初始资金</dt><dd>¥{number(request?.initial_cash_cny, 0)}</dd></div>
      <div><dt>现金保留</dt><dd>{percent(request?.minimum_cash_fraction)}</dd></div>
    </dl></section>
    <section><h3>评分因子</h3><div className="detail-rules">{request?.score_rules.map((rule) => <div key={rule.factor_id}><b>{rule.factor_id}</b><span>{rule.direction === 'HIGH' ? '高值优先' : '低值优先'} · 权重 {number(rule.weight)}%</span></div>)}</div></section>
    <section><h3>过滤规则</h3><div className="detail-rules">{request?.filter_rules.length ? request.filter_rules.map((rule) => <div key={rule.factor_id}><b>{rule.factor_id}</b><span>{rule.mode === 'EXCLUDE_HIGH' ? '排除最高' : '排除最低'} {percent(rule.fraction)} · {rule.missing_policy === 'EXCLUDE' ? '缺失排除' : '缺失保留'}</span></div>) : <span className="detail-none">无过滤规则</span>}</div></section>
...
export default function StrategyBacktestHistory({ onOpenRunning }: { onOpenRunning: () => void }) {
    ...
    {jobs.length > 0 && <div className="history-table">
      ...
      {jobs.map((job) => <div className={`history-table-item ${job.status.toLowerCase()}`} key={job.job_id}>
        <div className="history-main-row">
          <span className="history-name"><b>{cleanName(job.name)}</b><small>{dateTime(job.created_at)}</small></span>
        ...
        {expanded === job.job_id && <div className="history-detail-wrap">
          ...
          <div className="history-detail-actions">{job.status === 'RUNNING' ? <button onClick={onOpenRunning}>返回运行控制台 →</button> : <a href={strategyApi.reportUrl(job.job_id)} target="_blank">打开原始 JSON 报告 →</a>}</div>
        </div>}
      </div>)}
    </div>}
  </div>
