...
export default function RotationBacktest({ onOpenFactorCalculate, onOpenHistory }: { onOpenFactorCalculate: () => void; onOpenHistory: () => void }) {
  ...
  useEffect(() => {
    strategyApi.list().then(async ({ jobs }) => {
      const saved = window.localStorage.getItem(JOB_KEY)
      const active = jobs.find((item) => item.strategy_type === 'ROTATION' && ['QUEUED', 'RUNNING'].includes(item.status))
      const selected = active?.job_id || saved
      if (!selected) return
      const restored = await strategyApi.status(selected)
      if (restored.strategy_type === 'ROTATION') setJob(restored)
    }).catch(() => undefined)
  }, [])
  ...
  return <div className="strategy-page rotation-page">
    ...
    </div><aside className="strategy-console">
      <span>ROTATION CONTROL</span><h2>预检与运行</h2>
      <div className="strategy-summary"><p><span>候选组合</span><b>{candidates.length} 个</b></p><p><span>可用因子</span><b>{options?.factor_counts.calculated ?? '—'} 个</b></p><p><span>轮动规则</span><b>{lookback}日 / 阈值{threshold}%</b></p><p><span>调仓执行</span><b>次日开盘</b></p><p><span>区间</span><b>{start}<br />至 {end}</b></p></div>
      <button disabled={busy || candidates.length < 2 || Boolean(job && ['QUEUED', 'RUNNING'].includes(job.status))} onClick={() => run('preflight')}>一键预检</button>
      <div className="preview-control"><input type="date" value={previewDate} onChange={(e) => setPreviewDate(e.target.value)} /><button disabled={busy || candidates.length < 2 || Boolean(job && ['QUEUED', 'RUNNING'].includes(job.status))} onClick={() => run('preview')}>预览候选持仓</button></div>
      <button className="run-backtest" disabled={busy || candidates.length < 2} onClick={() => run('backtest')}>{busy ? '正在创建回测任务…' : job && ['QUEUED', 'RUNNING'].includes(job.status) ? '将当前配置加入等待队列' : '开始轮动回测'}</button>
      {preflight && <div className="strategy-ready"><b>✓ 数据预检通过</b><span>{preflight.session_count}个交易日 · 预计判断{preflight.estimated_decisions}次</span><small>行业 {preflight.industry.mapped_count}/{preflight.industry.universe_count} · 解决重叠 {preflight.industry.overlap_resolved_count}只</small></div>}
      {job && <div className={`rotation-job ${job.status.toLowerCase()}`}><b>{job.status === 'QUEUED' ? '等待队列中' : job.phase}</b><span>{job.status === 'QUEUED' ? `第 ${job.queue_position ?? '—'} 位 · 前面 ${job.jobs_ahead ?? 0} 个任务` : `${job.status} · ${job.progress}%`}</span>{job.status === 'QUEUED' && <button onClick={() => strategyApi.stop(job.job_id).then(setJob)}>取消排队</button>}{job.status === 'RUNNING' && <button onClick={() => strategyApi.stop(job.job_id).then(setJob)}>停止回测</button>}{job.status === 'PASS' && <button onClick={onOpenHistory}>进入历史结果</button>}</div>}
    </aside></div>
